...
type MeshPrimitive struct {
	...
	Mode       *DrawMode               `json:"mode,omitempty"`     //The topology type of primitives to render.
...
func (m *Mesh) GetPrimitive(i int) opt.O[MeshPrimitive] { return opt.At(m.Primitives, i) }

func (p *MeshPrimitive) GetIndices() opt.O[uint32]  { return opt.Of(p.Indices) }
func (p *MeshPrimitive) GetMaterial() opt.O[uint32] { return opt.Of(p.Material) }
func (p *MeshPrimitive) GetMode() DrawMode          { return opt.Of(p.Mode).Or(DrawMode_TRIANGLES) }
