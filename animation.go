...
type AnimationChannel struct {
	Property
	Sampler Id                     `json:"sampler"` //The index of a sampler in this animation used to compute the value for the target, e.g., a node's translation, rotation, or scale (TRS).
...
type AnimationChannelTarget struct {
	Property
	Node *Id  `json:"node,omitempty"` //The index of the node to animate. When undefined, the animated object **MAY** be defined by an extension.
...
type AnimationSampler struct {
	Property
	Input         Id            `json:"input"`                   //The index of an accessor containing keyframe timestamps. The accessor **MUST** be of scalar type with floating-point components. The values represent time in seconds with `time[0] >= 0.0`, and strictly increasing values, i.e., `time[n + 1] > time[n]`.
	Interpolation Interpolation `json:"interpolation,omitempty"` //Interpolation algorithm.
	Output        Id            `json:"output"`                  //The index of an accessor, containing keyframe output values.
}
