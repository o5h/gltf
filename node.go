...
type Node struct {
	...
	Matrix      *[16]float32 `json:"matrix,omitempty"`      //A floating-point 4x4 transformation matrix stored in column-major order.
	Mesh        *uint32      `json:"mesh,omitempty"`        //The index of the mesh in this node.
	Rotation    *[4]float32  `json:"rotation,omitempty"`    //The node's unit quaternion rotation in the order (x, y, z, w), where w is the scalar.
	...
	Weights     []float32    `json:"weights,omitempty"`     //The weights of the instantiated morph target. The number of array elements **MUST** match the number of morph targets of the referenced mesh. When defined, `mesh` **MUST** also be defined.
}
